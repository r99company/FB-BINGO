...
class LivePrizeTracker:
    """Detecta premios potenciales en tiempo real sin depender del verificador manual.

    Se carga una sola vez al comenzar/actualizar una partida y luego procesa cada
    bola mediante índices en memoria. Así 30.000 cartones no se vuelven a recorrer
    completos después de cada bola.
    """
    ...
    def load(self) -> None:
        ...
        for card in cards:
            ...
            series_id = self.repository.get_series_id_for_card(card.serial)
            _, card_index = self.repository.get_card_position(card.serial)
            self._series_position[card.serial] = (series_id, card_index)
