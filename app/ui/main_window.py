...
class TVWindow(QMainWindow):
    """Pantalla para TV/proyector con la misma identidad visual de FB-BINGO."""
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        header = QHBoxLayout()
        brand = QLabel("FB-BINGO"); brand.setObjectName("Brand")
        header.addWidget(brand); header.addStretch()
        self.card_title = QLabel("VERIFICACIÓN DE CARTÓN"); self.card_title.setObjectName("HeaderTitle")
        header.addWidget(self.card_title); layout.addLayout(header)
        self.number = QLabel("—"); self.number.setAlignment(Qt.AlignmentFlag.AlignCenter); self.number.setObjectName("CurrentBall")
        layout.addWidget(self.number, 2)
        self.card_panel = QFrame(objectName="Panel"); self.card_panel.setVisible(False)
        card_layout = QVBoxLayout(self.card_panel)
        self.card_serial = QLabel("CARTÓN —"); self.card_serial.setAlignment(Qt.AlignmentFlag.AlignCenter); self.card_serial.setObjectName("SectionTitle")
        ...
        for row in range(3):
            for column in range(9):
                cell = QLabel(""); cell.setAlignment(Qt.AlignmentFlag.AlignCenter); cell.setMinimumSize(65,42)
                self.card_cells.append(cell); self.card_grid.addWidget(cell,row,column)
        card_layout.addLayout(self.card_grid)
        self.card_result = QLabel(""); self.card_result.setAlignment(Qt.AlignmentFlag.AlignCenter); self.card_result.setObjectName("HeaderValue")
        card_layout.addWidget(self.card_result); layout.addWidget(self.card_panel)
        self.history = QLabel("Últimas: —"); self.history.setAlignment(Qt.AlignmentFlag.AlignCenter); self.history.setObjectName("HeaderValue")
        layout.addWidget(self.history)
        self.status = QLabel("FB-BINGO · LISTO PARA JUGAR"); self.status.setAlignment(Qt.AlignmentFlag.AlignCenter); self.status.setObjectName("Muted")
    ...
    def show_card_verification(self, card, called_numbers) -> None:
        verifier = CardVerifier(card); called = frozenset(called_numbers)
        lines = verifier.line_winners(called); bingo = verifier.is_bingo(called)
        self.card_panel.setVisible(True); self.card_serial.setText(f"CARTÓN {card.serial}")
        for index, cell in enumerate(self.card_cells):
            row, column = divmod(index,9); value = card.grid[row][column]
            ...
            cell.setStyleSheet(
                ...
                if value is not None and value in called else
                "background:#071A36;color:#FFFFFF;border:1px solid #174A86;border-radius:6px;font-size:22px;font-weight:900;"
            )
        if bingo: result = "★ BINGO ★"
        elif lines: result = "✓ LÍNEA " + ", ".join(str(row+1) for row in lines)
        else: result = "✕ NO HAY LÍNEA NI BINGO"
        self.card_result.setText(result); self.status.setText("FB-BINGO · VERIFICACIÓN DE CARTÓN")


class BingoMainWindow(QMainWindow):
    """Sala de locutora FB-BINGO 90 bolas, con el diseño final aprobado."""

    def __init__(self) -> None:
        super().__init__()
        self.setWindowTitle("FB-BINGO — Sala de Juego")
        self.resize(1540,930); self.setMinimumSize(1200,760)
        self.game = BingoGame(); self.repository = SQLiteSeriesRepository(database_path())
        self._buttons: dict[int,QPushButton] = {}; self.tv_window: TVWindow | None = None; self.generator_window: GeneratorWidget | None = None
        self._build_ui(); self._sync_ui()

    def _build_ui(self) -> None:
        root = QWidget(objectName="Root"); self.setCentralWidget(root); root.setStyleSheet(NEON_QSS)
        outer = QVBoxLayout(root); outer.setContentsMargins(10,10,10,10); outer.setSpacing(8)
        header = QFrame(objectName="TopBar"); hr = QHBoxLayout(header); hr.setContentsMargins(12,7,12,7)
        brand_box = QVBoxLayout(); brand_line = QHBoxLayout()
        b1=QLabel("FB-"); b1.setObjectName("Brand"); b2=QLabel("BINGO"); b2.setObjectName("BrandAccent")
        brand_line.addWidget(b1); brand_line.addWidget(b2); brand_line.addStretch(); brand_box.addLayout(brand_line)
        tagline=QLabel("SISTEMA PROFESIONAL DE BINGO 90 BOLAS"); tagline.setObjectName("Tagline"); brand_box.addWidget(tagline)
        hr.addLayout(brand_box,1)
        self.header_values=[]
        ...
        body=QHBoxLayout(); body.setSpacing(8); body.addWidget(self._build_sidebar()); body.addLayout(self._build_center(),1); body.addWidget(self._build_right_panel()); outer.addLayout(body,1); outer.addWidget(self._build_footer())

    def _build_sidebar(self) -> QFrame:
        sidebar=QFrame(objectName="Sidebar"); sidebar.setFixedWidth(190); lay=QVBoxLayout(sidebar); lay.setContentsMargins(8,10,8,8)
        title=QLabel("✦ FB-BINGO"); title.setObjectName("HeaderTitle"); lay.addWidget(title); lay.addSpacing(4)
    ...
    def _build_center(self) -> QVBoxLayout:
        ...
        top=QFrame(objectName="TopBar"); row=QHBoxLayout(top); t=QLabel("NÚMERO ACTUAL"); t.setObjectName("HeaderTitle"); row.addWidget(t); row.addStretch(); self.series_label=QLabel("SERIE — | CARTÓN — / 6"); self.series_label.setObjectName("HeaderTitle"); row.addWidget(self.series_label); content.addWidget(top)
        center=QHBoxLayout(); center.setSpacing(8)
        current=QFrame(objectName="Panel"); current.setFixedWidth(255); cl=QVBoxLayout(current)
        for text in ("NÚMERO ACTUAL",):
            cap=QLabel(text); cap.setObjectName("CurrentCaption"); cap.setAlignment(Qt.AlignmentFlag.AlignCenter); cl.addWidget(cap)
        ...
        self.history_label=QLabel("—"); self.history_label.setAlignment(Qt.AlignmentFlag.AlignCenter); self.history_label.setObjectName("HeaderValue"); self.history_label.setWordWrap(True); cl.addWidget(self.history_label)
        cap=QLabel("BOLAS CANTADAS"); cap.setObjectName("CurrentCaption"); cap.setAlignment(Qt.AlignmentFlag.AlignCenter); cl.addWidget(cap)
        self.count_label=QLabel("0\nDE 90"); self.count_label.setObjectName("Called"); self.count_label.setAlignment(Qt.AlignmentFlag.AlignCenter); cl.addWidget(self.count_label); center.addWidget(current)
    ...
    def _build_right_panel(self) -> QFrame:
        ...
        cap=QLabel("CONTROLES DEL JUEGO"); cap.setObjectName("CurrentCaption"); cap.setAlignment(Qt.AlignmentFlag.AlignCenter); lay.addWidget(cap)
        ...
        verify=QFrame(objectName="Panel"); vl=QVBoxLayout(verify); vt=QLabel("CARTÓN VERIFICADO"); vt.setObjectName("SectionTitle"); vl.addWidget(vt); self.verify_result=QLabel("—"); self.verify_result.setObjectName("HeaderValue"); self.verify_result.setWordWrap(True); vl.addWidget(self.verify_result)
        vr=QHBoxLayout(); self.card_serial_input=QLineEdit(); self.card_serial_input.setPlaceholderText("Número / serial del cartón"); self.card_serial_input.returnPressed.connect(self.verify_card); vb=QPushButton("VERIFICAR"); vb.setObjectName("Primary"); vb.clicked.connect(self.verify_card); vr.addWidget(self.card_serial_input); vr.addWidget(vb); vl.addLayout(vr); lay.addWidget(verify)
    ...
    def call_number(self, number: int) -> None:
        ...
        self.game.restore(GameState(drawn_numbers=self.game.history+(number,),remaining_numbers=tuple(remaining),paused=False)); self._sync_ui()
    ...
    def undo_number(self) -> None:
        ...
        history=self.game.history[:-1]; remaining=tuple(n for n in range(1,91) if n not in history)
        self.game.restore(GameState(drawn_numbers=history,remaining_numbers=remaining,paused=False)); self._sync_ui()

    def toggle_pause(self) -> None:
        if self.game.state.paused: self.game.resume()
        else: self.game.pause()
        self._sync_ui()

    def new_game(self) -> None:
        self.game.reset(); self._sync_ui()
    ...
    def verify_card(self) -> None:
        ...
        called=self.game.history; verifier=CardVerifier(card); lines=verifier.line_winners(called); bingo=verifier.is_bingo(called)
        result="★ BINGO ★" if bingo else ("✓ LÍNEA "+", ".join(str(r+1) for r in lines) if lines else "✕ NO HAY LÍNEA NI BINGO")
    ...
    def open_generator(self) -> None:
        if self.generator_window is None:
            self.generator_window=GeneratorWidget(self.repository); self.generator_window.setAttribute(Qt.WidgetAttribute.WA_DeleteOnClose,False)
    ...
    def _sync_ui(self) -> None:
        ...
        self.header_values[1].setText("PAUSADO" if state.paused else ("EN JUEGO" if count else "EN ESPERA")); self.header_values[3].setText(__import__("datetime").datetime.now().strftime("%d/%m/%Y  %H:%M")); self.header_values[2].setText("—"); self.header_values[4].setText("BINGO")
