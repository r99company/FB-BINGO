...
class BingoMainWindow(QMainWindow):
    ...
    def _build_ui(self):
        root=QWidget(objectName="Root"); self.setCentralWidget(root); root.setStyleSheet(NEON_QSS); outer=QVBoxLayout(root); body=QHBoxLayout(); body.addWidget(self._build_sidebar()); body.addLayout(self._build_center(),1); body.addWidget(self._build_right_panel()); outer.addLayout(body,1)
    def _build_sidebar(self):
        s=QFrame(objectName="Sidebar"); l=QVBoxLayout(s); items=(("🎙  PARTIDA","Operación del juego",None),("▣  GENERADOR","Generar e imprimir","generator"),("▣  PANTALLA TV","Pantalla para el público","tv"))
    ...
    def _build_center(self):
        c=QVBoxLayout(); row=QHBoxLayout(); t=QLabel("NÚMERO ACTUAL"); t.setObjectName("HeaderTitle"); row.addWidget(t); row.addStretch(); self.series_label=QLabel("SERIE — | CARTÓN — / 6"); self.series_label.setObjectName("HeaderTitle"); row.addWidget(self.series_label); w=QFrame(objectName="TopBar"); w.setLayout(row); c.addWidget(w)
        center=QHBoxLayout(); current=QFrame(objectName="Panel"); cl=QVBoxLayout(current); cap=QLabel("NÚMERO ACTUAL"); cap.setObjectName("CurrentCaption"); cl.addWidget(cap); self.current_label=QLabel("—"); self.current_label.setObjectName("CurrentBall"); self.current_label.setAlignment(Qt.AlignmentFlag.AlignCenter); cl.addWidget(self.current_label); self.call_state=QLabel("¡LISTO PARA JUGAR!"); self.call_state.setObjectName("HeaderValuePink"); self.call_state.setAlignment(Qt.AlignmentFlag.AlignCenter); cl.addWidget(self.call_state); self.history_label=QLabel("—"); self.history_label.setObjectName("HeaderValue"); self.history_label.setAlignment(Qt.AlignmentFlag.AlignCenter); cl.addWidget(self.history_label); self.count_label=QLabel("0\nDE 90"); self.count_label.setObjectName("Called"); self.count_label.setAlignment(Qt.AlignmentFlag.AlignCenter); cl.addWidget(self.count_label); center.addWidget(current)
    ...
    def undo_number(self):
        ...
        history=self.game.history[:-1]; self.game.restore(__import__('app.bingo.models',fromlist=['GameState']).GameState(drawn_numbers=history,remaining_numbers=tuple(n for n in range(1,91) if n not in history),paused=False)); self._sync_ui()
    def toggle_pause(self):
        self.game.resume() if self.game.state.paused else self.game.pause(); self._sync_ui()
    ...
    def open_tv(self):
        ...
        self.tv_window.show(); self.tv_window.update_game(self.game.current_number,self.game.last_five)
    ...
    def _sync_ui(self):
        s=self.game.state; cur=s.current_number; self.current_label.setText("—" if cur is None else str(cur)); self.count_label.setText(f"{len(s.drawn_numbers)}\nDE 90"); self.history_label.setText("  ".join(map(str,s.last_five[::-1])) if s.last_five else "—");
