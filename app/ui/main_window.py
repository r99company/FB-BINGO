...
class TVWindow(QMainWindow):
    """Pantalla pública: bola actual grande + tablero 1–90 + últimas bolas."""
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        header.addStretch()
        self.game_title = QLabel("PARTIDA RÁPIDA")
        self.game_title.setObjectName("HeaderTitle")
        header.addWidget(self.game_title)
        layout.addLayout(header)

        content = QHBoxLayout()
        content.setSpacing(10)
        current_panel = QFrame(objectName="Panel")
        current_panel.setFixedWidth(255)
        current_layout = QVBoxLayout(current_panel)
        caption = QLabel("NÚMERO ACTUAL")
        caption.setObjectName("CurrentCaption")
        ...
        self.number.setMinimumHeight(190)
        current_layout.addWidget(self.number)
        ...
        current_layout.addWidget(self.call_state)
        history_caption = QLabel("ÚLTIMAS 5 BOLAS")
        history_caption.setObjectName("CurrentCaption")
        history_caption.setAlignment(Qt.AlignmentFlag.AlignCenter)
        current_layout.addWidget(history_caption)
        self.history = QLabel("—")
        self.history.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.history.setObjectName("HeaderValue")
        self.history.setWordWrap(True)
        current_layout.addWidget(self.history)
        count_caption = QLabel("BOLAS JUGADAS")
        count_caption.setObjectName("CurrentCaption")
        count_caption.setAlignment(Qt.AlignmentFlag.AlignCenter)
        current_layout.addWidget(count_caption)
        self.count = QLabel("0 / 90")
        self.count.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.count.setObjectName("Called")
        current_layout.addWidget(self.count)
        content.addWidget(current_panel)

        board_panel = QFrame(objectName="Panel")
        board_layout = QVBoxLayout(board_panel)
        board_title = QLabel("TABLERO DE BINGO · 90 BOLAS")
        board_title.setObjectName("SectionTitle")
        board_layout.addWidget(board_title)
        self.board_buttons: dict[int, QPushButton] = {}
        board = QGridLayout()
        board.setSpacing(4)
    ...
    def update_game(self, number: int | None, history: tuple[int, ...]) -> None:
        ...
        self.call_state.setText("¡CANTADO!" if number is not None else "¡LISTO PARA JUGAR!")
        self.history.setText(" · ".join(map(str, history[::-1])) if history else "—")
        self.count.setText(format_ball_count(len(history)))
        called = set(history)
        for value, button in self.board_buttons.items():
            button.setProperty("called", value in called)
            button.setProperty("current", value == number)
    ...
    def show_card_verification(self, card, called_numbers) -> None:
        """Muestra una verificación en la TV sin perder el tablero público."""
        ...
        self.game_title.setText(f"VERIFICACIÓN · CARTÓN {card.serial}")
        self.number.setText("BINGO" if bingo else ("LÍNEA" if lines else "—"))
        self.status.setText("★ BINGO ★" if bingo else ("✓ LÍNEA" if lines else "✕ SIN PREMIO"))
...
class BingoMainWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        hr.setContentsMargins(12, 7, 12, 7)
        brand_box = QVBoxLayout()
        brand_line = QHBoxLayout()
        b1 = QLabel("FB-")
        b1.setObjectName("Brand")
        b2 = QLabel("BINGO")
        b2.setObjectName("BrandAccent")
        brand_line.addWidget(b1)
        brand_line.addWidget(b2)
        ...
        self.header_values = []
        for title, value, pink in (
            ("JUEGO ACTUAL", "PARTIDA RÁPIDA", False),
            ("ESTADO DEL JUEGO", "EN ESPERA", False),
            ("SERIE ACTUAL", "—", True),
            ("FECHA Y HORA", "—", True),
        ):
            ...
            small = QLabel(title)
            small.setObjectName("HeaderSmall")
            val = QLabel(value)
            val.setObjectName("HeaderValuePink" if pink else "HeaderValue")
            val.setAlignment(Qt.AlignmentFlag.AlignCenter)
            lay.addWidget(small)
            lay.addWidget(val)
            hr.addWidget(card, 1)
            self.header_values.append(val)
    ...
    def _build_sidebar(self) -> QFrame:
        ...
        for text, sub, action in items:
            ...
            btn.setProperty("active", text.startswith("🎙"))
            if action == "tv":
                btn.clicked.connect(self.open_tv)
            elif action == "generator":
                btn.clicked.connect(self.open_generator)
            lay.addWidget(btn)
        lay.addStretch()
        ok = QLabel("● SISTEMA CONECTADO\n   Listo para jugar")
        ok.setObjectName("StatusGood")
        lay.addWidget(ok)
        return sidebar
    ...
    def _build_center(self) -> QVBoxLayout:
        ...
        row = QHBoxLayout(top)
        t = QLabel("NÚMERO ACTUAL")
        t.setObjectName("HeaderTitle")
        row.addWidget(t)
        row.addStretch()
        self.series_label = QLabel("SERIE — | CARTÓN — / 6")
        ...
        current.setFixedWidth(275)
        cl = QVBoxLayout(current)

        cap = QLabel("NÚMERO ACTUAL")
        cap.setObjectName("CurrentCaption")
        cap.setAlignment(Qt.AlignmentFlag.AlignCenter)
        cl.addWidget(cap)
        self.current_label = QLabel("—")
        self.current_label.setObjectName("CurrentBall")
        ...
        self.call_state.setObjectName("HeaderValuePink")
        self.call_state.setAlignment(Qt.AlignmentFlag.AlignCenter)
        cl.addWidget(self.call_state)

        cap = QLabel("ÚLTIMAS 5 BOLAS")
        cap.setObjectName("CurrentCaption")
        cap.setAlignment(Qt.AlignmentFlag.AlignCenter)
        cl.addWidget(cap)
        self.history_label = QLabel("—")
        self.history_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.history_label.setObjectName("HeaderValue")
        self.history_label.setWordWrap(True)
        cl.addWidget(self.history_label)

        cap = QLabel("BOLAS JUGADAS")
        cap.setObjectName("CurrentCaption")
        cap.setAlignment(Qt.AlignmentFlag.AlignCenter)
        cl.addWidget(cap)
        self.count_label = QLabel("0 / 90")
        self.count_label.setObjectName("Called")
        self.count_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        cl.addWidget(self.count_label)

        digit_title = QLabel("DIGITAR BOLA EXTRAÍDA")
        digit_title.setObjectName("CurrentCaption")
        digit_title.setAlignment(Qt.AlignmentFlag.AlignCenter)
        cl.addWidget(digit_title)
        digit_hint = QLabel("Digite la bola física y presione ENTER")
        digit_hint.setObjectName("PublicHint")
        digit_hint.setAlignment(Qt.AlignmentFlag.AlignCenter)
        digit_hint.setWordWrap(True)
        cl.addWidget(digit_hint)
        entry = QHBoxLayout()
        self.ball_input = QLineEdit()
        self.ball_input.setObjectName("BallInput")
        self.ball_input.setPlaceholderText("1–90")
        self.ball_input.setMaxLength(2)
        self.ball_input.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.ball_input.setInputMethodHints(Qt.InputMethodHint.ImhDigitsOnly)
        self.ball_input.returnPressed.connect(self.enter_ball)
        entry.addWidget(self.ball_input, 2)
        enter = QPushButton("ENTER")
        enter.setObjectName("Primary")
        enter.clicked.connect(self.enter_ball)
        entry.addWidget(enter, 1)
        cl.addLayout(entry)
        self.ball_message = QLabel("LISTO · ESPERANDO BOLA FÍSICA")
        self.ball_message.setObjectName("Muted")
        self.ball_message.setWordWrap(True)
        self.ball_message.setAlignment(Qt.AlignmentFlag.AlignCenter)
        cl.addWidget(self.ball_message)
        center.addWidget(current)

        board_panel = QFrame(objectName="Panel")
        bl = QVBoxLayout(board_panel)
        bt = QLabel("TABLERO DE BINGO · 90 BOLAS")
        ...
        for number in range(1, 91):
            ...
            btn.setObjectName("Ball")
            btn.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Expanding)
            btn.clicked.connect(lambda checked=False, n=number: self.call_number(n))
            self._buttons[number] = btn
            grid.addWidget(btn, (number - 1) // 10, (number - 1) % 10)
        bl.addLayout(grid, 1)
        center.addWidget(board_panel, 1)
        content.addLayout(center, 1)

        actions = QHBoxLayout()
        draw = QPushButton("▶ SORTEO AUTOMÁTICO\nF1")
        draw.setObjectName("Secondary")
        draw.setToolTip("Modo automático. En operación normal use DIGITAR BOLA.")
        ...
        finish = QPushButton("■ FINALIZAR\nF4")
        finish.setObjectName("Danger")
        finish.clicked.connect(self.new_game)
        for btn in (draw, self.pause_button, undo, finish):
    ...
    def _build_footer(self) -> QFrame:
        ...
        u = QLabel("◉ USUARIO\n   LOCUTORA")
        u.setObjectName("FooterText")
        e = QLabel("▣ EQUIPO\n   PC-LOCUTORA-01")
        e.setObjectName("FooterText")
        v = QLabel("★ VERSIÓN\n   1.0.0")
        v.setObjectName("FooterText")
        s = QLabel("● SISTEMA CONECTADO")
        s.setObjectName("StatusGood")
        row.addWidget(u)
        row.addSpacing(30)
        row.addWidget(e)
        row.addStretch()
    ...
    def enter_ball(self) -> bool:
        ...
        try:
            ...
            self.ball_message.setText("✕ DIGITE UN NÚMERO DEL 1 AL 90")
            self.ball_input.selectAll()
            self.ball_input.setFocus()
            return False
        if not 1 <= number <= 90:
            self.ball_message.setText("✕ NÚMERO INVÁLIDO · USE 1–90")
            self.ball_input.selectAll()
            self.ball_input.setFocus()
            return False
        if number in self.game.history:
            self.ball_message.setText(f"✕ LA BOLA {number} YA FUE CANTADA")
            self.ball_input.selectAll()
            self.ball_input.setFocus()
            return False
        if self.game.state.finished:
            self.ball_message.setText("✕ LA PARTIDA ESTÁ FINALIZADA")
            self.ball_input.selectAll()
            self.ball_input.setFocus()
            return False
        ...
        self.ball_message.setText(f"✓ BOLA {number} REGISTRADA · TABLERO ACTUALIZADO")
        self._sync_ui()
        self.ball_input.setFocus()
        return True
    ...
    def draw_number(self) -> None:
        ...
        try:
            ...
            return
        self.ball_message.setText("MODO AUTOMÁTICO · BOLA SORTEADA POR EL PROGRAMA")
        self._sync_ui()
    ...
    def call_number(self, number: int) -> None:
        ...
        self.ball_message.setText(f"✓ BOLA {number} REGISTRADA")
        self._sync_ui()
    ...
    def undo_number(self) -> None:
        ...
        self.ball_message.setText("↶ ÚLTIMA BOLA DESHECHA · LISTA PARA DIGITAR")
        self._sync_ui()
        self.ball_input.setFocus()
    ...
    def toggle_pause(self) -> None:
        if self.game.state.paused:
            self.game.resume()
            self.ball_message.setText("✓ PARTIDA REANUDADA")
        else:
            self.game.pause()
            self.ball_message.setText("Ⅱ PARTIDA PAUSADA")
        self._sync_ui()
    ...
    def _sync_ui(self) -> None:
        ...
        self.count_label.setText(format_ball_count(count))
        ...
        self.header_values[1].setText("PAUSADO" if state.paused else ("EN JUEGO" if count else "EN ESPERA"))
        self.header_values[3].setText(datetime.now().strftime("%d/%m/%Y  %H:%M"))
        self.header_values[2].setText("—")
        ...
        if self.tv_window is not None:
            self.tv_window.update_game(current, self.game.last_five)
        self.ball_input.setFocus()
