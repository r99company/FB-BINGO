...
QPushButton#Ball:hover { background:#0D315D; border-color:#22DFFF; } QPushButton#Ball[called="true"] { background:#D91B83; border:2px solid #FF58B5; color:#FFFFFF; }
...
QLineEdit:focus { border:2px solid #FF3FA4; } QLabel#InputHint { color:#AFC7E8; font-size:11px; }
...
class TVWindow(QMainWindow):
    def __init__(self,parent=None):
        ...
        h=QHBoxLayout(); b=QLabel("FB-BINGO"); b.setObjectName("Brand"); h.addWidget(b); h.addStretch(); self.card_title=QLabel("VERIFICACIÓN DE CARTÓN"); self.card_title.setObjectName("HeaderTitle"); h.addWidget(self.card_title); layout.addLayout(h)
        ...
        self.card_panel=QFrame(objectName="Panel"); self.card_panel.setVisible(False); cl=QVBoxLayout(self.card_panel); self.card_serial=QLabel("CARTÓN —"); self.card_serial.setObjectName("SectionTitle"); cl.addWidget(self.card_serial); self.card_grid=QGridLayout(); self.card_cells=[]
        for r in range(3):
            for c in range(9):
                cell=QLabel(""); cell.setAlignment(Qt.AlignmentFlag.AlignCenter); cell.setMinimumSize(65,42); self.card_cells.append(cell); self.card_grid.addWidget(cell,r,c)
        cl.addLayout(self.card_grid); self.card_result=QLabel(""); self.card_result.setObjectName("HeaderValue"); cl.addWidget(self.card_result); layout.addWidget(self.card_panel)
        self.history=QLabel("Últimas: —"); self.history.setAlignment(Qt.AlignmentFlag.AlignCenter); self.history.setObjectName("HeaderValue"); layout.addWidget(self.history); self.status=QLabel("FB-BINGO · LISTO PARA JUGAR"); self.status.setAlignment(Qt.AlignmentFlag.AlignCenter); self.status.setObjectName("Muted"); layout.addWidget(self.status)
    ...
    def show_card_verification(self,card,called_numbers):
        verifier=CardVerifier(card); called=frozenset(called_numbers); lines=verifier.line_winners(called); bingo=verifier.is_bingo(called); self.card_panel.setVisible(True); self.card_serial.setText(f"CARTÓN {card.serial}")
        for i,cell in enumerate(self.card_cells):
            r,c=divmod(i,9); value=card.grid[r][c]; cell.setText("" if value is None else str(value)); cell.setStyleSheet("background:#D91B83;color:#FFFFFF;" if value is not None and value in called else "background:#071A36;color:#FFFFFF;")
...
class BingoMainWindow(QMainWindow):
    ...
    def _build_center(self):
        c=QVBoxLayout(); top=QFrame(objectName="TopBar"); r=QHBoxLayout(top); t=QLabel("NÚMERO ACTUAL"); t.setObjectName("HeaderTitle"); r.addWidget(t); r.addStretch(); self.series_label=QLabel("SERIE — | CARTÓN — / 6"); r.addWidget(self.series_label); c.addWidget(top)
        center=QHBoxLayout(); current=QFrame(objectName="Panel"); cl=QVBoxLayout(current); cap=QLabel("NÚMERO ACTUAL"); cap.setObjectName("CurrentCaption"); cl.addWidget(cap); self.current_label=QLabel("—"); self.current_label.setObjectName("CurrentBall"); self.current_label.setAlignment(Qt.AlignmentFlag.AlignCenter); cl.addWidget(self.current_label); self.call_state=QLabel("¡LISTO PARA JUGAR!"); self.call_state.setObjectName("HeaderValuePink"); cl.addWidget(self.call_state); h=QLabel("ÚLTIMAS 5 BOLAS"); h.setObjectName("CurrentCaption"); cl.addWidget(h); self.history_label=QLabel("—"); self.history_label.setObjectName("HeaderValue"); self.history_label.setAlignment(Qt.AlignmentFlag.AlignCenter); cl.addWidget(self.history_label); h=QLabel("BOLAS CANTADAS"); h.setObjectName("CurrentCaption"); cl.addWidget(h); self.count_label=QLabel("0\nDE 90"); self.count_label.setObjectName("Called"); self.count_label.setAlignment(Qt.AlignmentFlag.AlignCenter); cl.addWidget(self.count_label); center.addWidget(current)
        board=QFrame(objectName="Panel"); bl=QVBoxLayout(board); bl.addWidget(QLabel("TABLERO DE BINGO · 90 BOLAS")); g=QGridLayout();
        for n in range(1,91): b=QPushButton(str(n)); b.setObjectName("Ball"); b.setSizePolicy(QSizePolicy.Policy.Expanding,QSizePolicy.Policy.Expanding); b.clicked.connect(lambda checked=False,x=n:self.call_number(x)); self._buttons[n]=b; g.addWidget(b,(n-1)//10,(n-1)%10)
        bl.addLayout(g); center.addWidget(board,1); c.addLayout(center,1); return c
    def _build_right_panel(self):
        p=QFrame(objectName="Panel"); l=QVBoxLayout(p); cap=QLabel("DIGITAR BOLA EXTRAÍDA"); cap.setObjectName("CurrentCaption"); l.addWidget(cap); hint=QLabel("La máquina física extrae la bola.\nDigite aquí el número y presione ENTER."); hint.setObjectName("InputHint"); l.addWidget(hint); self.ball_input=QLineEdit(); self.ball_input.setObjectName("BallInput"); self.ball_input.setPlaceholderText("Digite la bola (1-90)"); self.ball_input.setMaxLength(2); self.ball_input.setAlignment(Qt.AlignmentFlag.AlignCenter); self.ball_input.returnPressed.connect(self.enter_ball); l.addWidget(self.ball_input); enter=QPushButton("ENTER"); enter.setObjectName("Primary"); enter.clicked.connect(self.enter_ball); l.addWidget(enter); self.ball_message=QLabel("LISTO · ESPERANDO BOLA FÍSICA"); l.addWidget(self.ball_message); return p
    def enter_ball(self):
        ...
        except Exception as exc: self.ball_message.setText(f"✕ {exc}"); self.ball_input.selectAll(); self.ball_input.setFocus(); return False
    ...
    def undo_number(self):
        ...
        history=self.game.history[:-1]; self.game.restore(GameState(drawn_numbers=history,remaining_numbers=tuple(n for n in range(1,91) if n not in history),paused=False)); self._sync_ui()
    ...
    def _build_footer(self):
        f=QFrame(objectName="BottomBar"); r=QHBoxLayout(f); r.addWidget(QLabel("◉ USUARIO\n   LOCUTORA")); r.addStretch(); r.addWidget(QLabel("★ VERSIÓN\n   1.0.0")); return f
    def _sync_ui(self):
        ...
        if self.tv_window:self.tv_window.update_game(cur,s.last_five)
