...
def _init_with_operational_modules(self: BingoMainWindow) -> None:
    _original_init(self)
    self._finalized = False; self.cartons_window = None; self.generator_window = None; self.sales_window = None; self.verification_window = None; self.reports_window = None; self.settings_window = None
    ...
    for button in self.findChildren(QPushButton):
        if button.text().startswith(("▣  GENERADOR", "▣ GENERADOR")):
            ...
            button.setProperty("fb_bingo_finish_button", True); _replace_signal_connection(button.clicked, self.finalize_game)
...
def main(argv=None) -> int:
    ...
    try:
        ...
        window = BingoMainWindow(); window.show(); return app.exec()
    except Exception as exc:
