...
def _enter_ball_with_history(self) -> bool:
    if getattr(self, "_finalized", False):
        self.ball_message.setText("✕ PARTIDA FINALIZADA · INICIE UNA NUEVA PARTIDA"); self.ball_input.selectAll(); self.ball_input.setFocus(); return False
    if self.game.state.paused:
        self.ball_message.setText("Ⅱ PARTIDA PAUSADA · NO SE PUEDE DIGITAR"); self.ball_input.selectAll(); self.ball_input.setFocus(); return False
...
def _finalize_game_with_history(self) -> None:
    ...
    self._finalized = True; self.game.resume(); self.header_values[1].setText("FINALIZADA"); self.ball_message.setText("✓ PARTIDA FINALIZADA · EXCEL GENERADO" if export_error is None else "✓ PARTIDA FINALIZADA · EXCEL NO GENERADO"); self.ball_input.clear(); self.ball_input.setEnabled(False); self._sync_ui(); self.header_values[1].setText("FINALIZADA"); _publish_tv(self); _set_finish_button_mode(self, True)
...
def _install_operator_shortcuts(self: BingoMainWindow) -> None:
    ...
    for sequence, callback in (("F1", self.draw_number), ("F2", self.toggle_pause), ("F3", self.undo_number), ("F4", lambda: self.new_game() if getattr(self, "_finalized", False) else self.finalize_game)):
        shortcut = QShortcut(QKeySequence(sequence), self); shortcut.setContext(Qt.ShortcutContext.WindowShortcut); shortcut.activated.connect(callback); self._operator_shortcuts.append(shortcut)


def _install_model_selector(self: BingoMainWindow) -> None:
    self.model_selector = GameModelSelector(self); self.model_selector.set_change_guard(lambda: not self.game.history or getattr(self, "_finalized", False)); top = self.series_label.parentWidget(); row = top.layout(); row.insertWidget(max(0, row.count() - 1), self.model_selector)


def _init_with_operational_modules(self: BingoMainWindow) -> None:
    _original_init(self); self._finalized = False; self.cartons_window = None; self.generator_window = None; self.sales_window = None; self.verification_window = None; self.reports_window = None; self.settings_window = None
    self.history_repository = SQLiteGameHistoryRepository(database_path()); self.history_service = GameHistoryService(self.history_repository); self.history_game_id = None; _install_model_selector(self)
    settings = SettingsService(application_data_dir() / "settings.json"); self.tv_sync_server = GameSyncServer(host="0.0.0.0", port=int(settings.get("tv_server_port", 8765))); self.tv_sync_thread = threading.Thread(target=self.tv_sync_server.serve_forever, daemon=True); self.tv_sync_thread.start(); self.tv_sync_client = GameSyncClient(str(settings.get("tv_server_host", "127.0.0.1")), int(settings.get("tv_server_port", 8765)))
    self.open_cartons = lambda: _open_cartons(self); self.open_sales = lambda: _open_sales(self); self.open_verification = lambda: _open_verification(self); self.open_reports = lambda: _open_reports(self); self.open_settings = lambda: _open_settings(self); self.open_tv = lambda: _open_tv(self); self.enter_ball = lambda: _enter_ball_with_history(self); self.draw_number = lambda: _draw_with_history(self); self.call_number = lambda number: _call_with_history(self, number); self.undo_number = lambda: _undo_with_history(self); self.toggle_pause = lambda: _pause_with_history(self); self.finalize_game = lambda: _finalize_game_with_history(self); self.new_game = lambda: _new_game_with_history(self)
