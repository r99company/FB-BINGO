        ...
        client.publish(
            {
                ...
                else ("PAUSADA" if self.game.state.paused else "EN CURSO"),
            }
        )
...
def _enter_ball_with_history(self) -> bool:
    if getattr(self, "_finalized", False):
        ...
        self.ball_input.selectAll(); self.ball_input.setFocus(); return False
...
def _draw_with_history(self) -> None:
    if getattr(self, "_finalized", False):
        self.ball_message.setText("✕ PARTIDA FINALIZADA · INICIE UNA NUEVA PARTIDA"); return
...
def _call_with_history(self, number: int) -> None:
    if getattr(self, "_finalized", False):
        self.ball_message.setText("✕ PARTIDA FINALIZADA · INICIE UNA NUEVA PARTIDA"); return
    if self.game.state.paused:
        self.ball_message.setText("Ⅱ PARTIDA PAUSADA · NO SE PUEDE CANTAR BOLA"); return
    _original_call_number(self, number); _history_sync(self); _publish_tv(self)


def _undo_with_history(self) -> None:
    if getattr(self, "_finalized", False):
        self.ball_message.setText("✕ PARTIDA FINALIZADA · NO SE PUEDE DESHACER"); return
    was_paused = self.game.state.paused
    _original_undo_number(self)
    if was_paused:
        self.game.pause(); self._sync_ui()
    _history_sync(self); _publish_tv(self)
...
def _set_finish_button_mode(self: BingoMainWindow, new_game: bool) -> None:
    ...
    if button is None:
        for candidate in self.findChildren(QPushButton):
            if candidate.property("fb_bingo_finish_button") is True:
...
def _finalize_game_with_history(self) -> None:
    ...
    self.ball_input.clear(); self.ball_input.setEnabled(False); self._sync_ui(); self.header_values[1].setText("FINALIZADA")
    _publish_tv(self); _set_finish_button_mode(self, True)


def _new_game_with_history(self) -> None:
    if not getattr(self, "_finalized", False): _finalize_game_with_history(self)
    _original_new_game(self); self._finalized = False; self.ball_input.setEnabled(True)
    self.ball_message.setText("NUEVA PARTIDA · ESPERANDO BOLA FÍSICA"); self._sync_ui(); self.header_values[1].setText("EN ESPERA")
    _start_history_game(self); _set_finish_button_mode(self, False); _publish_tv(self)
...
def _install_operator_shortcuts(self: BingoMainWindow) -> None:
    ...
    shortcuts = (
        ...
        ("F4", lambda: _f4_action(self)), ("Ctrl+4", lambda: _f4_action(self)),
        ("Ctrl+5", self.verify_card),
    )
...
def _apply_operator_visual_refresh(self: BingoMainWindow) -> None:
    """Aligera la pantalla sin alterar la lógica de la partida."""
    ...
    if current is not None:
        current.setFixedSize(156, 156)
    state = getattr(self, "call_state", None)
    if state is not None:
        state.setText("BOLA CANTADA" if self.game.current_number is not None else "LISTO PARA JUGAR")
...
def _install_model_selector(self: BingoMainWindow) -> None:
    ...
    self.model_selector.set_change_guard(lambda: not self.game.history or getattr(self, "_finalized", False))
    top = self.series_label.parentWidget(); row = top.layout()
...
def _init_with_operational_modules(self: BingoMainWindow) -> None:
    ...
    self.cartons_window = None; self.generator_window = None; self.sales_window = None; self.verification_window = None
    self.reports_window = None; self.settings_window = None
    self.history_repository = SQLiteGameHistoryRepository(database_path())
    self.history_service = GameHistoryService(self.history_repository); self.history_game_id = None
    ...
    self.open_cartons = lambda: _open_cartons(self); self.open_sales = lambda: _open_sales(self)
    self.open_verification = lambda: _open_verification(self); self.open_reports = lambda: _open_reports(self)
    self.open_settings = lambda: _open_settings(self); self.open_tv = lambda: _open_tv(self)
    self.enter_ball = lambda: _enter_ball_with_history(self); self.draw_number = lambda: _draw_with_history(self)
    self.call_number = lambda number: _call_with_history(self, number); self.undo_number = lambda: _undo_with_history(self)
    self.toggle_pause = lambda: _pause_with_history(self); self.finalize_game = lambda: _finalize_game_with_history(self)
    self.new_game = lambda: _new_game_with_history(self)
    _start_history_game(self); wire_operational_controls(self)
    for button in self.findChildren(QPushButton):
        if button.text().startswith(("▣  GENERADOR", "▣ GENERADOR")):
    ...
    if getattr(self, "finish_button", None) is not None:
        self.finish_button.setProperty("fb_bingo_finish_button", True); _replace_signal_connection(self.finish_button.clicked, self.finalize_game)
...
def _run_self_test() -> int:
    game = BingoGame(); repository = SQLiteSeriesRepository(database_path())
    print("FB-BINGO OK"); print(f"bola_actual={game.current_number}"); print(f"series_en_bd={repository.count_series()}")
    return 0


def run_tv_mode() -> int:
    app = QApplication(sys.argv); settings = SettingsService(application_data_dir() / "settings.json")
    window = TVWindow(); client = GameSyncClient(str(settings.get("tv_server_host", "127.0.0.1")), int(settings.get("tv_server_port", 8765)))
    timer = QTimer(window)
    def poll() -> None:
        try:
            state = client.get_state(); window.update_game(state.get("current"), tuple(state.get("history", []))[:5])
...
def main(argv=None) -> int:
    ...
    try:
        ...
        _show_startup_error(exc); return 1
