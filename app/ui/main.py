...
def _publish_tv(self: BingoMainWindow) -> None:
    """Publica para TV usando el mismo estado operativo cuando existe sincronización."""
    client = getattr(self, "station_sync_client", None) or getattr(self, "tv_sync_client", None)
    if client is None:
        return
    try:
...
def _finalize_game_with_history(self) -> None:
    ...
    self.ball_message.setText(
        "✓ PARTIDA FINALIZADA · REPORTE GENERADO"
        if export_error is None
        else "✓ PARTIDA FINALIZADA · REPORTE NO GENERADO"
    )
...
def _init_with_operational_modules(self: BingoMainWindow) -> None:
    ...
    peer_port = int(settings.get("peer_port") or settings.get("tv_server_port", 8765))

    self.tv_sync_server = GameSyncServer(host="0.0.0.0", port=peer_port)
    self.tv_sync_thread = threading.Thread(target=self.tv_sync_server.serve_forever, daemon=True)
    self.tv_sync_thread.start()

    # La sincronización operativa es la fuente única del estado de juego.
    # tv_sync_client queda solo para compatibilidad con el modo TV heredado.
    self.tv_sync_client = GameSyncClient(
        str(settings.get("tv_server_host", peer_host)),
        int(settings.get("tv_server_port", peer_port)),
    )
    ...
    self._set_finish_button_mode = lambda new_game: _set_finish_button_mode(self, new_game)
    ...
    for button in self.findChildren(QPushButton):
        ...
        elif button.text().startswith(("🛒  VENTAS", "🛒 VENTAS")):
            _replace_signal_connection(button.clicked, self.open_sales)
        elif button.text().startswith(("✓  VERIFICACIÓN", "✓ VERIFICACIÓN")):
            _replace_signal_connection(button.clicked, self.open_verification)
        elif button.text().startswith(("▥  REPORTES", "▥ REPORTES")):
            _replace_signal_connection(button.clicked, self.open_reports)
        elif button.text().startswith(("⚙  CONFIGURACIÓN", "⚙ CONFIGURACIÓN")):
            _replace_signal_connection(button.clicked, self.open_settings)
        elif button.text().startswith(("▣  PANTALLA TV", "▣ PANTALLA TV")):
            _replace_signal_connection(button.clicked, self.open_tv)

    if getattr(self, "finish_button", None) is not None:
        self.finish_button.setProperty("fb_bingo_finish_button", True)
        _replace_signal_connection(self.finish_button.clicked, self.finalize_game)
    ...
    def poll() -> None:
        try:
            ...
            window.status.setText(
                f"FB-BINGO · {state.get('status', 'EN CURSO')} · {state.get('game', 'PARTIDA RÁPIDA')}"
            )
        except (OSError, ValueError, TimeoutError):
...
def _show_startup_error(exc: Exception) -> None:
    try:
        QMessageBox.critical(None, "FB-BINGO", f"No se pudo iniciar FB-BINGO:\n\n{exc}")
    except Exception:
