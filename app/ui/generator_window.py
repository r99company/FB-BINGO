...
class GeneratorWidget(QWidget):
    """Generación nueva y reimpresión separadas para evitar reutilizaciones accidentales."""
    ...
    def _build_ui(self) -> None:
        ...
        self.series_count_label = QLabel(); self.series_count_label.setObjectName("Muted")
        ...
        self.card_count.valueChanged.connect(self._on_count_changed)
        ...
        reprint_button.setObjectName("Secondary"); reprint_button.clicked.connect(self.load_for_reprint)
        ...
        form.addRow(self.generate_button); form.addRow(reprint_button); form.addRow(print_button)
        form.addRow(preview); form.addRow(save)
        ...
        advanced_form.addRow("Impresión", self.duplicate_column); advanced_form.addRow(self.logo, logo_button)
        advanced.setVisible(False); advanced_toggle.toggled.connect(advanced.setVisible); form.addRow(advanced)

        info = QLabel("GENERAR NUEVA PRODUCCIÓN nunca reutiliza un rango existente. CARGAR / REIMPRIMIR EXISTENTES recupera exactamente los mismos cartones. Las series físicas son de 6 y la capacidad llega a 30.000 cartones.")
    ...
    def _on_count_changed(self) -> None:
        self._update_range_label()
        try:
            start = self.production_service.next_generation_start(self.card_count.value())
            if self._loaded_start_card is None or self.start_card.value() + self.card_count.value() - 1 < start:
                self.start_card.setValue(start)
        except ValueError:
            pass
    ...
    def _update_range_label(self) -> None:
        start, count = self.start_card.value(), self.card_count.value(); end = start + count - 1
        if end > self.production_service.max_cards:
            self.range_label.setText("Supera la capacidad configurada"); self.series_count_label.setText("—"); self.pages_label.setText("—"); return
        if count % 6:
            self.range_label.setText(f"{start:,} – {end:,}"); self.series_count_label.setText("Cantidad debe ser múltiplo de 6"); self.pages_label.setText("—"); return
        series = count // 6
        pages = series if self.duplicate_column.isChecked() else (series + 1) // 2 if hasattr(self, "duplicate_column") else (series + 1) // 2
        self.range_label.setText(f"{start:,} – {end:,} ({count:,} cartones)"); self.series_count_label.setText(f"{series:,}"); self.pages_label.setText(f"{pages:,}")
    ...
    def _requested_range(self) -> tuple[int, int, int]:
        start, count = self.start_card.value(), self.card_count.value(); end = start + count - 1
        if end > self.production_service.max_cards: raise ValueError(f"El rango supera la capacidad configurada de {self.production_service.max_cards:,} cartones")
        if count % 6: raise ValueError("La cantidad de cartones debe ser múltiplo de 6")
        return start, end, count

    def _load_requested_cards(self, start_card: int, end_card: int) -> None:
        self._cards = self.repository.get_cards_range(start_card, end_card); self._loaded_start_card = start_card; self._loaded_card_count = end_card - start_card + 1; self._svg = ""

    def generate_series(self) -> None:
        try:
            start, end, count = self._requested_range(); model = CardModel(self.model.currentData())
            self.generate_button.setEnabled(False); self.generate_button.setText("GENERANDO…"); self.preview_label.setText(f"Generando {count:,} cartones nuevos…"); QApplication.processEvents()
            lot = self.production_service.create_new_lot(start, end, model=model, operator="generador-ui")
            ...
            self.preview_label.setText(f"LISTO · {result.card_count:,} cartones nuevos · {result.series_count:,} series de 6 · rango {start:,}–{end:,}.")
        except DuplicateProductionError as exc: QMessageBox.warning(self, "No se pudo completar la generación", str(exc))
        except (ValueError, RuntimeError, KeyError) as exc: QMessageBox.warning(self, "No se pudo generar", str(exc))
        finally: self.generate_button.setEnabled(True); self.generate_button.setText("GENERAR NUEVA PRODUCCIÓN")

    def load_for_reprint(self) -> None:
        try:
            start, end, count = self._requested_range(); self._load_requested_cards(start, end); self._render_preview()
            self.preview_label.setText(f"CARGADO · {count:,} cartones existentes · rango {start:,}–{end:,}. Son exactamente los mismos cartones.")
        except (ValueError, KeyError, OSError) as exc: QMessageBox.warning(self, "No se pudo cargar el rango", str(exc))

    def _cards_for_page(self, offset: int) -> tuple[tuple[BingoCard, ...], tuple[BingoCard, ...] | None]:
        if not self._cards: raise ValueError("No hay cartones cargados")
        left = self._cards[offset:offset + 6]
        if len(left) != 6: raise ValueError("La producción no contiene una serie completa de 6 cartones")
    ...
    def _render_preview(self) -> None:
        if len(self._cards) < 6: raise ValueError("No hay una serie completa cargada")
        renderer = A4SvgRenderer(style=self._style()); left, right = self._cards_for_page(0)
        ...
        self.preview_widget.load(self._svg.encode("utf-8")); self._update_range_label()

    def preview_a4(self) -> None:
        try:
            start, _, count = self._requested_range()
            if self._loaded_start_card != start or self._loaded_card_count != count: raise ValueError("Primero genera o carga el rango que deseas visualizar")
            self._render_preview(); self.preview_label.setText("Vista A4 · primera hoja del rango cargado · 12 posiciones")
        except (ValueError, OSError) as exc: QMessageBox.warning(self, "Error de vista previa", str(exc))

    def print_a4(self) -> None:
        try:
            start, _, count = self._requested_range()
            if self._loaded_start_card != start or self._loaded_card_count != count: raise ValueError("Primero genera o carga el rango que deseas imprimir")
            ...
            try:
                ...
                for offset in range(0, count, step):
                    ...
                    if not svg_renderer.isValid(): raise RuntimeError(f"No se pudo preparar la hoja A4 que comienza en el cartón {start + offset:,}")
            ...
            self.preview_label.setText(f"IMPRESIÓN COMPLETADA · {pages:,} hojas A4 · {count:,} cartones físicos por columna de producción.")
    ...
    def save_a4(self) -> None:
        try:
            start, _, count = self._requested_range()
            if self._loaded_start_card != start or self._loaded_card_count != count: raise ValueError("Primero genera o carga el rango que deseas guardar")
            if not self._svg: self._render_preview()
            path, _ = QFileDialog.getSaveFileName(self, "Guardar hoja A4", f"fb_bingo_cartones_{start}-{start + 5}.svg", "SVG (*.svg)")
