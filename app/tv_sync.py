...
def _merge_history(local: tuple[int, ...], remote: tuple[int, ...]) -> tuple[int, ...]:
    ...
    if remote == local[:len(remote)]:
        return local
    # Revisiones iguales con historiales divergentes representan eventos
    # concurrentes. No existe un orden causal fiable; usamos un orden
    # determinista para garantizar convergencia entre ambas estaciones.
    return tuple(sorted(set(local).union(remote)))
