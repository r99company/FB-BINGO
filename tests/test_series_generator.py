...
def test_each_card_has_five_numbers_per_row_and_one_or_two_per_column() -> None:
    for model in (CardModel.A, CardModel.B):
        series = SeriesGenerator(seed=456).generate(f"SER-{model.value}", model)
...
def test_numbers_stay_in_their_bingo_column_ranges() -> None:
    ...
    series = SeriesGenerator(seed=321).generate("SER-RANGES", CardModel.B)
    for card in series.cards:
