...
def test_default_a4_card_size_is_97x45_5_mm_for_two_series_per_page():
    layout = A4PrintLayout()
    slots = layout.card_slots()
    assert layout.card_width_mm == 97.0
    assert layout.card_height_mm == 45.5
    assert abs(slots[0].width / MM_TO_PT - 97.0) < 0.01
    assert abs(slots[0].height / MM_TO_PT - 45.5) < 0.01
