...
def test_export_game_history_creates_operational_excel(tmp_path: Path):
    ...
    export_game_history(output, game_name="Partida rápida", series="SERIE-001", called_numbers=[47, 12, 90, 5])
    assert output.exists()
    workbook = load_workbook(output)
    assert workbook.sheetnames == ["Partidas"]
    sheet = workbook["Partidas"]
    assert [sheet.cell(1, column).value for column in range(1, 6)] == ["Fecha y hora", "Juego", "Serie", "Bolas cantadas", "Historial"]
